...
// object (taught in server/modules/missions/runtime/agent-operator-prompt.ts; the server mirrors this
// validation in server/modules/missions/runtime/agent-spec-framing.ts, which gates commit_draft — the
// same parser-pair split the spec-draft protocol already uses between
// server/modules/specs/runtime/spec-draft-parser.ts and client/src/lib/spec-draft.ts). The block is
