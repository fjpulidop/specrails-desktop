...
vi.mock('../../../context/WebViewModalContext', () => ({ useWebViewModal: () => ({ openWebView: vi.fn(), canOpenWebView: false }) }))
vi.mock('../../../context/AgentChatContext', () => ({ useAgentChat: () => ({ applyPrDecisionSnapshot: () => 'applied' }) }))
vi.mock('../../../hooks/useMilestoneProgress', () => ({ useStackedHeadDeliveryIds: () => new Set() }))
vi.mock('../../../hooks/useAgentRefActions', () => ({ useAgentRefActions: () => ({ openRef: vi.fn() }) }))
vi.mock('../../../hooks/useRunVitals', () => ({
...
vi.mock('../../../lib/git-refresh', () => ({ notifyGitChanged: vi.fn() }))
vi.mock('../../JobDetailModal', () => ({ JobDetailModal: () => null }))

const runtime = vi.hoisted(() => ({
  state: { runs: [] as unknown[], error: '', busy: null as string | null, answers: {}, setAnswer: vi.fn(), act: vi.fn(async () => {}), refresh: vi.fn() },
  calls: [] as unknown[],
}))
vi.mock('../../job-run/useRuntimeRuns', () => ({
