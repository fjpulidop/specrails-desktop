...
4. Start from the [server module guide](server/modules/README.md),
   [frontend feature guide](client/src/features/README.md) or [CLI guide](cli/README.md).
   For an unfamiliar path, search [the generated source map](docs/internals/source-map.md).
...
| Queue, cancellation, recovery | [execution core](server/modules/execution/README.md), [queue-manager.ts](server/modules/execution/runtime/queue-manager.ts), [interactive-job-session.ts](server/modules/execution/runtime/interactive-job-session.ts), [spawn-lifecycle.ts](server/modules/execution/runtime/spawn-lifecycle.ts) | [interactive jobs](docs/internals/interactive-jobs.md), [startup recovery](docs/internals/startup-recovery-audit.md) |
| Loop execution | [loop-run-manager.ts](server/modules/loops/runtime/loop-run-manager.ts), [loop-executors.ts](server/modules/loops/runtime/loop-executors.ts), [loop-graph.ts](server/modules/loops/runtime/loop-graph.ts), [loop-runs-store.ts](server/modules/loops/runtime/loop-runs-store.ts) | [loop logs](docs/internals/loop-step-log-explorer.md), [companion contract](docs/internals/companion-rails-as-loops-contract.md) |
| Rails and PR decisions | [delivery policy](server/modules/delivery/README.md), [rails-router.ts](server/modules/delivery/runtime/rails-router.ts), [rail-isolated-launch.ts](server/modules/delivery/runtime/rail-isolated-launch.ts), [rail-pr-decision.ts](server/modules/delivery/runtime/rail-pr-decision.ts), [rail-merge-orchestrator.ts](server/modules/delivery/runtime/rail-merge-orchestrator.ts) | [safe PR flow](docs/internals/safe-pr-review-flow.md), [review packet](docs/internals/review-packet.md) |
| AI providers and runtime | [provider contract](server/providers/types.ts), [registry](server/providers/registry.ts), [runtime bridge](server/modules/agent-runtime/runtime/agent-runtime-bridge.ts), [runtime settings](server/modules/agent-runtime/runtime/agent-runtime-settings.ts) | [adding providers](docs/internals/adding-a-provider.md), [programmatic runtime](docs/internals/programmatic-agent-runtime.md) |
| Chats and missions | [conversation core](server/modules/conversations/README.md), [chat-manager.ts](server/modules/conversations/runtime/chat-manager.ts), [agent-chat-manager.ts](server/modules/missions/runtime/agent-chat-manager.ts), [AgentChatContext](client/src/features/missions/context/AgentChatContext.tsx) | [mission rail cards](docs/internals/mission-rail-cards.md), [MCP mission audit](docs/internals/mcp-mission-audit.md) |
| Specs, tickets, project builder | [ticket-store.ts](server/modules/specs/runtime/ticket-store.ts), [project-router-tickets.ts](server/project-router-tickets.ts), [milestone-chain.ts](server/modules/builder/runtime/milestone-chain.ts), [DashboardPage](client/src/features/dashboard/pages/DashboardPage.tsx) | [project builder](docs/internals/project-builder.md), [spec addenda](docs/internals/spec-addenda.md) |
| Accounting and telemetry | [ai-invocations.ts](server/modules/accounting/runtime/ai-invocations.ts), [spending.ts](server/modules/accounting/runtime/spending.ts), [result-event.ts](server/modules/accounting/runtime/result-event.ts), [integer allocation](server/util/distribute-int.ts) | [cost audit](COST-ACCOUNTING-AUDIT.md), [analytics audit](ANALYTICS-AUDIT-REPORT.md) |
| Browser and code exploration | [browser-capture-manager.ts](server/modules/browser/runtime/browser-capture-manager.ts), [browser-playwright.ts](server/modules/browser/runtime/browser-playwright.ts), [code-explorer-router.ts](server/modules/code/runtime/code-explorer-router.ts) | [browser performance](docs/internals/browser-capture-performance.md), [login popups](docs/internals/browser-login-popups.md), [file stories](docs/internals/code-explorer-story.md) |
...
| Jira and integrations | [Jira sync](server/jira/jira-sync-manager.ts), [Jira client](server/jira/jira-client.ts), [IntegrationsPage](client/src/features/integrations/pages/IntegrationsPage.tsx) | [configuration](docs/internals/configuration.md) |
...
- Use a core's public `index.ts` or a focused public subpath declared in the
  [server](server/modules/boundaries.json) / [frontend](client/src/features/boundaries.json)
  boundary manifests. Avoid eager barrels that initialize unrelated adapters.
  Composition roots wire infrastructure; do not add undeclared cross-feature imports.
- Keep domain rules pure. Application code depends on narrow, use-case-owned ports.
  ...
  or explicitly effectful runtime coordinators.
...
- Add a module dependency rule with the extraction; see [architecture tests](server/modules/architecture.test.ts).
  For module additions, update the public API, reviewed boundary manifest, module README
  and source map. Do not regenerate a manifest to bypass the fixed core rules.
...
| Server feature | `npx vitest run server/modules/<capability>` plus affected route/lifecycle tests |
...
| Capability dependencies | `npm run audit:architecture`; review manifest changes |
