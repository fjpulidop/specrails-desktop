...
describe('mobile mission contract', () => {
  ...
  it('bounds long history and excludes provider/context internals and arbitrary system card payloads', () => {
    ...
    const cards = missionSnapshot({ messages: [{ id: 'card', role: 'system', content: JSON.stringify({ kind: 'pr_decision', decision: 'on_review', ticketIds: [17], runIds: ['run-1'], runtime: {status: 'running', currentStep: 'Developer', secret: 'private'}, executionManifest: { secret: 'private' } }) }] })
    expect(cards.messages[0]).toMatchObject({ content: '', card: { decision: 'on_review', ticketIds: [17], runIds: ['run-1'], runtime: {status: 'running', currentStep: 'Developer'} } })
