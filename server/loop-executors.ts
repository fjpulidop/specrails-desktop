...
export function createLoopExecutors(
  ...
  return {
    ...
    async runAiStep({ runtimeDeveloperOverride, coreRun, prompt, sessionId, provider, model, effort, profileName, cwd, repoDir, executionManifest, onLine, onRawLine, onSpawn, aiStepTimeoutMs, idleTimeoutMs }) {
      ...
      if (coreRun?.implementation && core) {
        ...
        return runAgentRuntimeInvocation({
          ...
          defaultProvider: provider, developerOverride: runtimeDeveloperOverride,
