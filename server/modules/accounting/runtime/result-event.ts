...
export function finaliseInvocationResult(
  ...
  if (nativeCostAbsent && hasBillableUsage) {
    ...
    if (computed !== null) {
      ...
      // is to add the rate to server/modules/accounting/runtime/pricing.ts, per its quarterly-review
      // contract). We deliberately do NOT fabricate a fallback rate.
      console.warn(
        `[pricing] no rate-card entry for "${adapter.id}:${cloned.model}" — ` +
          `cost will be persisted NULL. Add it to server/modules/accounting/runtime/pricing.ts PRICING.`,
      )
    }
  }
