...
export async function probeDefinitionRun(ctx: DefinitionProbeContext, runId: string): Promise<DefinitionRunProbe> {
...
export async function probeDefinitionRuns(ctx: DefinitionProbeContext, runIds: string[]): Promise<Map<string, DefinitionRunProbe>> {
  const results = new Map<string, DefinitionRunProbe>(), ids = [...new Set(runIds)]
  let cursor = 0
  await Promise.all(Array.from({ length: Math.min(4, ids.length) }, async () => {
    for (;;) { const id = ids[cursor++]; if (id === undefined) return; results.set(id, await probeDefinitionRun(ctx, id)) }
  }))
  return results
}
