...
it.skipIf(!pairedCore || !existsSync(path.join(pairedCore, 'dist/agent-runtime/cli.js')))('reads an actual paused Core definition through the shared status contract', async () => {
  ...
  const full = await probeDefinitionRun(ctx(), 'run', true)
  expect(full.status).toBe('paused')
  expect(full.scopes).toEqual(expect.arrayContaining([expect.objectContaining({ nodePath: 'ask', scopeId: 'root', kind: 'question', attemptId: expect.any(String) })]))
