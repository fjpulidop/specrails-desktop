...
import { promoteAgentProtocolFences } from '../../missions/runtime/agent-fence-promotion'
...
describe('client mirror parity', () => {
  it('client/src/features/rails/lib/rail-launch-draft.ts is byte-identical except for the mirror note and host import paths', () => {
    const server = fs.readFileSync(path.join(__dirname, 'rail-launch-parser.ts'), 'utf8')
    const client = fs.readFileSync(path.join(require('node:path').resolve(__dirname, '../../..'), '..', 'client', 'src', 'features', 'rails', 'lib', 'rail-launch-draft.ts'), 'utf8')
    const norm = (s: string) => s
      .replace(/lives at [^\n]+ and MUST stay byte-identical/, 'MIRROR')
      // Capability ownership changes the host-relative import path, not the
      // imported bindings or the protocol implementation compared below.
      .replace(/from '[^']*\/(json-tolerant|pr-follow-up-scope)'/g, "from '$1'")
    expect(norm(client)).toBe(norm(server))
  })

  it('client/src/features/specs/lib/spec-addenda-core.ts is byte-identical to server/modules/specs/runtime/spec-addenda-core.ts', () => {
    const server = fs.readFileSync(path.join(__dirname, '../../specs/runtime/spec-addenda-core.ts'), 'utf8')
    const client = fs.readFileSync(path.join(require('node:path').resolve(__dirname, '../../..'), '..', 'client', 'src', 'features', 'specs', 'lib', 'spec-addenda-core.ts'), 'utf8')
...
describe('followUp on a proposal (pr-follow-up-fixes)', () => {
  ...
  it('client/src/features/delivery/lib/pr-follow-up-scope.ts is byte-identical to server/modules/delivery/runtime/pr-follow-up-scope.ts except for the mirror note', () => {
    const server = fs.readFileSync(path.join(__dirname, 'pr-follow-up-scope.ts'), 'utf8')
    const client = fs.readFileSync(path.join(require('node:path').resolve(__dirname, '../../..'), '..', 'client', 'src', 'features', 'delivery', 'lib', 'pr-follow-up-scope.ts'), 'utf8')
