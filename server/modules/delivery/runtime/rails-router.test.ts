...
import os from 'os'
import fs from 'fs'
import { mutateStore, readStore } from '../../specs/runtime/ticket-store'
import { buildSpecAddendum, readSpecAddenda } from '../../specs/runtime/spec-addenda'
...
describe('rails-router POST /pr-decision', () => {
  ...
  it('run-only cards (run:<id>) accept ONLY dismiss, need conversationId, and resolve through the agent-chat manager', async () => {
    const { setAgentChatManager } = await import('../../missions/runtime/agent-chat-registry')
...
import { setAgentChatManager } from '../../missions/runtime/agent-chat-registry'
import type { AgentChatManager } from '../../missions/runtime/agent-chat-manager'
