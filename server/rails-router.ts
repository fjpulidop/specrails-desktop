import { validateRuntimeDeveloperOverride } from './agent-runtime-settings'
...
export function createRailsRouter(): Router {
  ...
  router.post('/:railIndex/launch', async (req: Request, res: Response) => {
    ...
    let runtimeDeveloperOverride
    try { runtimeDeveloperOverride = validateRuntimeDeveloperOverride(req.body?.runtimeDeveloperOverride) }
    catch { res.status(400).json({ error: 'invalid_developer_override' }); return }
