...
function serializeInstallConfigYaml(config: Record<string, unknown>): string {
  ...
  const requestedProvider = typeof c.provider === 'string' ? c.provider : ''
  const provider = getProviderDefault(requestedProvider)
    ? requestedProvider as SpecProvider
    : 'claude'
  const defaultModel = getProviderDefault(provider) || getProviderDefault('claude')
  ...
  const lines = [
    ...
    `  defaults: { model: ${c.models?.defaults?.model ?? defaultModel} }`,
