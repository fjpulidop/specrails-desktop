...
describe('loops-router factory loops', () => {
  it('lists the built-in factory loops with their mode + graph', async () => {
    ...
    expect(ids).toEqual(['factory:implement', 'factory:batch', 'factory:freestyle', 'factory:openspec'])
    ...
    expect(freestyle.claudeOnly).toBe(true)
    expect(freestyle.graph.nodes.length).toBeGreaterThan(0)
