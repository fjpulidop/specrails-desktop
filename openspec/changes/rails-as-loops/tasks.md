...
- [x] 1.2 Add a NATIVE command kind to the catalog and register `{{cmd:ultracode}}` — expands to a raw autonomous prompt (NOT a slash command); flagged claude-only. (Factory ultracode routes to QueueManager's real `_buildUltracodePrompt` in §3; this is the custom-loop fallback prompt.)
- [x] 1.3 Add `ticketScope: 'all' | 'per-ticket'` to `LoopCommand` (implement/batch=`all`, ultracode=`per-ticket`) + `dominantTicketScope()` + `referencesClaudeOnlyCommand()` helpers.
...
- [x] 1.5 Unit tests: catalog expansion (implement/batch all-ids, codex `$`, ultracode raw), scope + claude-only helpers, `{{spec.ids}}` vs `{{spec.id}}`. 46 server tests green, typecheck clean.

## 2. Factory loops (the first deliverable)

- [x] 2.1 Define the `implement`, `batch`, `ultracode` factory loops in `server/loop-factory.ts` (graphs via `aiLoopGraph` using `{{cmd:implement}}`/`{{cmd:batch}}`/`{{cmd:ultracode}}` + `{{cmd:verify}}`); each carries its legacy `mode` + id `factory:<name>` + claudeOnly for ultracode; helpers `getFactoryLoop`/`isFactoryLoopId`/`factoryLoopMode`/`factoryLoopForMode`. 5 tests; graphs validate.
...
- [x] 3.5 Claude-only guard: a custom loop referencing a claude-only command (`{{cmd:ultracode}}`) on a non-claude rail → 400. (Factory ultracode keeps the existing `mode==='ultracode'` claude guard.)
...
- [x] 4.3 Play guarded until an effective loop exists; Ultracode built-in hidden on non-claude rails + `handleEngineChange` falls the rail back to Implement when leaving claude on Ultracode.
- [x] 4.4 i18n `railControls.builtInLoops`/`customLoops` ×8 (parity green); built-in names reuse existing `railControls.implement|batch|ultra`.
- [x] 4.5 Client tests: RailLoopSelector (built-ins always, ultracode gating, custom-when-enabled, onChange), rail-loops helpers (mode⇄id round-trip), RailControls (segmented control gone), all 19 page suites (211) green.

## 5. Standalone run for ticket-less loops

- [x] 5.1 `loopNeedsTicket(graph)` (client `lib/loop-ticket-need.ts`) — true on `{{spec.*}}` or `{{cmd:implement|batch|ultracode}}`; tested.
