...
Rails today launch a spec in one of three modes (implement / batch-implement / freestyle), each a single AI-CLI invocation that runs to completion once. There is no iterative, self-correcting execution and no reusable notion of a "workflow". This change adds **Loops**: a global library of visually-authored, app-governed iterative workflows, plus a 4th rail mode that runs a published loop against the dragged spec.
...
Constraint confirmed by spike: `@xyflow/react@12.11.1` installs and renders under React 19.2.7 (zustand → 4.5.7, no peer conflict). Constraint confirmed by code map: a `loop` mode requires **zero specrails-core changes** (raw prompts, like freestyle).
