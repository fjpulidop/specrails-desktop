...
2. **The modes already ARE loops.** `rails-router` builds `/specrails:implement #1 #2 #3 --yes` over ALL the rail's tickets for both `implement` and `batch-implement` (only the command name differs); `ultracode` is a per-ticket raw autonomous prompt (`_buildUltracodePrompt`). These map cleanly onto loop graphs driven by commands.
...
- `implement` / `batch` / `ultracode` shipped as factory loops, expressed via catalog commands.
...
**Non-Goals:**
- Full engine merge (re-running implement/batch/ultracode THROUGH `LoopRunManager`). Deferred — factory loops keep using the battle-tested QueueManager path.
...
`implement` / `batch` / `ultracode` ship as built-in loop definitions surfaced in the Loops gallery as read-only (locked) with a "Fork to edit" action that clones into an editable user draft. They are NOT user-editable in place (their behaviour is contractually tied to the core commands).
...
A catalog command declares `ticketScope: 'all' | 'per-ticket'`. `implement`/`batch` = `all` (one run, inject `{{spec.ids}}` = `#1 #2 #3`); `ultracode` = `per-ticket` (one run per ticket, inject `{{spec.id}}`). The launch path reads the loop's command(s) to decide how many runs to spawn and which ticket token to fill.
...
### D4 — `{{cmd:ultracode}}` is a "native/raw" command, not a slash command
The catalog grows a command kind beyond `coreCommand` (slash) and `template` (curated prompt): a **native** command whose expansion is the raw autonomous prompt (`_buildUltracodePrompt`'s shape), claude-only. Resolution gates: a loop containing a native claude-only command forces `provider: 'claude'` and no profile (mirrors today's ultracode guard in rails-router).
**Alternative**: model ultracode as a `template` curated prompt. **Rejected** — it must reuse the exact, tested ultracode prompt-building, not a paraphrase.
...
- **Ultracode claude-only inside a unified picker** → The rail must reject/disable an ultracode loop when the rail's provider isn't claude (reuse the existing guard), with a clear message.
