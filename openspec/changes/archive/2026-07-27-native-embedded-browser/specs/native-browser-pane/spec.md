...
### Requirement: Session persistence and popup coercion
The native pane SHALL persist cookies/logins across app restarts (Windows via a dedicated profile directory under `~/.specrails/`; macOS via the app's default persistent WebKit data store). `window.open`/popup requests SHALL be coerced into a same-pane navigation to the requested URL.
...
#### Scenario: Popup coerced to same-pane navigation
- **WHEN** page content calls `window.open(url)`
- **THEN** the pane itself navigates to `url` and no new OS window appears
