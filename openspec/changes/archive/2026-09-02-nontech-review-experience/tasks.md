...
- [x] 3.1 Additive migration: `revision_note` + `revision_of` columns on `rail_pr_deliveries` (metadata on the NEW generation row; no decision enum changes)
- [x] 3.2 Guard exemption in `rails-router.ts` (`prDeliveryRevisionAllowed`: names the rail's ACTIVE delivery + non-terminal + exact full ticket set ⇒ allowed; everything else keeps the 409, and a wrong id gets a distinct `invalid_revision_target`). MCP: rather than mirroring the guard into `launch_all` (where skipping a pending decision is the CORRECT batch behaviour — a fan-out must never silently revise), the single `launch` action gained the `revisionOfDeliveryId`/`revisionNote` params and the route re-validates. Tests: valid revision, subset, wrong id, missing note, malformed id, ordinary launch still 409s.
- [x] 3.3 Revision generation path: launch creates a superseding generation via `createPrDeliveryGeneration` carrying the sentence; failed revision restores predecessor (existing machinery) — integration tests for supersede + restore + wire (only known decision values broadcast)
- [x] 3.4 Factory `revision` loop (4th factory loop, desktop-only, Architect-less): distilled revision command ai-step — goal = sentence + injected durable context (spec snapshot, PR-body digest, branch diff summary, harvested evidence), instructions orchestrate an `sr-reviewer` pass over the resulting diff producing `confidence-score.json` (agents present via worktree overlay; zero core changes) — + standard `{{cmd:verify}}`; flows through `classifyLoopEffect`/isolated launch unchanged; branch resume onto the ticket's existing branch verified by test
- [x] 3.4b Post-PR door uses the same loop: the router forces `factory:revision` for ANY revision launch regardless of the rail's stored mode, and `prDeliveryRevisionAllowed` accepts every non-terminal decision (so `pr_draft`/`pr_ready` revisions take the same loop through the existing continuation machinery). Covered by the loop-selection test + the non-terminal-decision matrix in `prDeliveryRevisionAllowed`.
- [x] 3.5 Fresh-seed contract: `server/modules/execution/runtime/revision-seed.ts` builds the durable briefing (instruction + frozen spec + branch + what the previous run REPORTED, failure-first), wired as the per-run `REVISION_REQUEST` constant. Opportunistic `--resume` DROPPED as unnecessary complexity: the branch carries the work, the seed carries the intent, and the previous session's cwd (a released worktree) makes resume unreliable by construction — the house pattern (loop-back drops, contract-refine fresh retry) already prefers fresh+durable.
- [x] 3.6 Packet revision input: "What would you change?" box → flagged launch; expected-cost line only from real revision-run history; disabled states (already deciding, operation lease held); i18n ×8
- [x] 3.7 Version lineage UI on the packet: v1/v2/… chain from `supersedes_delivery_id`, each with its sentence; "Back to version 1" verb riding restore; failed-revision packet variant ("could not be applied — version 1 still on review", sentence preserved for retry)
- [x] 3.8 Drift nudges: triggers (cumulative cost >50% of original, majority out-of-set churn from `file_story_contributions`, count ≥3 backstop); advisory banner with real numbers + one-click "turn notes into an updated spec" agent handoff; trigger logged; never blocks
...
- [x] 3.10 Flag `SPECRAILS_DELIVERY_REVISIONS` (default on, opt-out); kill-switch test = byte-identical legacy guard
- [x] 3.11 Gates: typecheck, vitest, coverage green
...
- [x] 5.1 specrails-core ask FILED: PR fjpulidop/specrails-core#333 (`feat/structured-verification-counts`), an OpenSpec change for a `verification-results.json` artifact. CORRECTION to this task as written: delta-scoped verification ALREADY shipped in core 5.0.0 (`sr-reviewer` § "Verification policy (scoped-first)", via `pipeline-cost-economy`), so only the structured counts were outstanding. The core spec also reuses the metric names `health-check.md` already defines, and — because the reviewer's step 4 delegates to the developer's pass of record — requires every entry to declare `scope` and `ran_by`, so desktop can never present a scoped subset as a full-suite result. Desktop-side consumption is deliberately deferred until the artifact exists.
