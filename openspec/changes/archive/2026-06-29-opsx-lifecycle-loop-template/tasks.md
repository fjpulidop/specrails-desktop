...
- [x] 1.1 Add `opsx:ff`, `opsx:apply`, `opsx:verify` as `providerNative` `LoopCommand`s in `server/modules/loops/runtime/loop-command-catalog.ts` (claude/gemini → `/opsx:<name>`, codex → `$opsx:<name>`, plus a `template` fallback prompt). Do NOT model them as `coreCommand`.
...
- [x] 2.1 Add a run-scoped captured-variable store to the loop run state in `server/modules/loops/runtime/loop-run-manager.ts` (map of `{{run.<name>}}` values, empty until captured).
