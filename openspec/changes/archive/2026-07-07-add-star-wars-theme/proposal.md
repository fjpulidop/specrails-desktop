...
- Update `client/src/features/settings/lib/__tests__/themes.test.ts` fixed-array/fixed-list assertions (`THEME_IDS` exact-equality check, the dark-themes list, the `scheme` list) to include `star-wars`.
...
- **Affected files**: `client/src/features/settings/lib/themes.ts`, `client/src/globals.css`, `client/src/components/theme-effects/LightsaberTrail.tsx` (new), `client/src/features/settings/components/theme-effects/ThemeEffectLayer.tsx`, `server/desktop-router.ts`, `client/src/features/settings/lib/__tests__/themes.test.ts`, all 8 `client/src/locales/<lang>/settings.json` files.
