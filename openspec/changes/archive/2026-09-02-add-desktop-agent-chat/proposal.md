...
- **Conversational context palette.** The chat input supports a single immediate palette with three typed entry points plus one visual entry point: `@` for structured Specrails references (projects, specs, jobs, mission artifacts, conversation-created items), `#` for operational traces and numbered/history items (jobs, runs, errors, PRs/checks), `/` for actions, and a composer `+` button that opens the same system for discoverability. Results are scoped by the current mission/project first, then conversation-created objects, recents, active state, and finally global search. Selecting a result inserts a live chip carrying `{type, id, label, scope}` so the agent receives resolved context rather than ambiguous text, and the sent conversation bubble keeps the selected `@`, `#`, or `/` token as a self-contained chip after refresh.
...
- `agent-chat-context-palette`: the input-level `@`/`#`/`/` palette plus composer `+` button, inline composer chips, structured reference chips, keyboard-first selection, disambiguation, scoped retrieval across conversation-created and existing Specrails objects, and context-aware action catalog.
...
- **Client (context palette)**: chat composer tokenization, floating palette, result grouping, keyboard navigation, inline composer chips, live chips, preview popovers, and an optional visual context button that opens the same `@` palette for discoverability.
- **Server/API (context palette)**: lightweight lookup endpoints or MCP-backed resolvers for mentionable entities/traces, structured reference payloads attached to agent turns, and conversation artifact indexing for items created during the current mission.
