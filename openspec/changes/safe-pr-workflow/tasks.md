...
- [ ] 4.2 **specrails-core**: same one-rule guard for `backend-developer.md`/`frontend-developer.md` self-commit (`git add -A && git commit` → skip when `SPECRAILS_GIT_AUTO=false`); document `SPECRAILS_GIT_AUTO` in the desktop↔core integration contract. **Core README** (apply WITH the code so the doc never runs ahead of behaviour): near the existing `SPECRAILS_PROFILE_PATH` env-var note (~line 204), add — *"`SPECRAILS_GIT_AUTO` — when set to `false`, `/specrails:implement` skips its Ship phase (no branch/commit/push/PR). specrails-desktop sets it so the desktop layer owns version control (it delivers the draft PR itself)."*
- [x] 4.3 **desktop**: injects `SPECRAILS_GIT_AUTO=false` into every rail spawn when `SPECRAILS_RAIL_DELIVER_PR` is on — both the loop-engine path (`loop-executors.ts`) and the QueueManager fallback path (`queue-manager.ts`, covers implement/batch/freestyle). The signal travels with the invocation (survives into a worktree), superseding the fragile backlog-config stopgap. Gated OFF ⇒ no change.
