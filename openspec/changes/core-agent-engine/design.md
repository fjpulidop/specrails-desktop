...
### Recorded topology and event-contract acceptance

The run explorer renders Core's public topology rather than inferring execution
edges from event order. It is read-only, navigates component bodies, and selects
exact node/scope/attempt log segments. Per-scope latest status is aggregated without
allowing a successful parallel branch to hide a failure. The component uses the
same theme contract as Loop Builder and refits the viewport at each drill-down.

Transport acceptance includes a checked-in JSONL recording generated by actual
Core/LangGraph with a deterministic local executor, not hand-authored transport
