...
- D0 normaliza nextNodePath, conserva engineVersion y lee métricas superiores; deriva roles del config congelado cuando status no incluye catálogo. La validación de rutas separa sintaxis de pertenencia exacta al run.
...
### CI, release and testing optimization decision — 2026-09-26

The user requested optimization of the entire testing and release path. Server coverage is split into three isolated single-worker shards and client coverage into four shards, with mandatory same-commit, same-lock, disjoint/exhaustive inventory and checksum verification before merging. Global thresholds are evaluated only against the complete coverage map using the unchanged normal configurations. The quality gate preserves the verified platform-independent frontend; native release jobs reuse exactly these assets from trusted source CI and retain all platform-specific build/signing/smoke controls. Real fixture tests demonstrate that missing/tampered reports and actual uncovered functions fail; remote wall-time improvement must be measured rather than assumed.
