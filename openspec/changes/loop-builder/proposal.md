...
Users today launch a spec on a rail in one of three modes (implement / batch-implement / ultracode) — each a single, fire-and-forget AI invocation. There is no way to express an **iterative, self-correcting workflow** ("keep implementing and running the tests until they pass", "verify the build, and if it fails, fix and re-verify"). Closed-loop agent workflows are a proven pattern (cf. loops.elorm.xyz), but no first-class, reusable, app-governed mechanism exists.
...
- **New `loop` rail mode** (4th mode alongside implement / batch-implement / ultracode). Selecting it reveals: published-loop picker + AI engine (Claude/Codex) + model + reasoning effort. The rail's spec flows into the loop as `{{spec.title}}` / `{{spec.description}}` context.
...
- **specrails-core**: ZERO coupling. Loops use raw prompts (like ultracode's `_buildUltracodePrompt` → `claude -p`), no slash command, no core-version gate.
