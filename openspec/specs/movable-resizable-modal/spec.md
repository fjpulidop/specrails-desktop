# movable-resizable-modal Specification

## Purpose

Provide a single reusable `useMovableResizableModal()` client hook that grants any centered modal panel the ability to be freely moved (by header drag) and resized (by corner/edge grips), while remaining byte-identical to its pre-change appearance until the user first interacts. The hook owns the geometry state, pointer-capture lifecycle, viewport gating, off-screen jail guard, keyboard/accessibility for grips, and pointer-capture safety — so individual modals adopt floating behavior without re-implementing any of it.
