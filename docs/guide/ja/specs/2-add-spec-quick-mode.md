...
> **Kimi は Quick Spec では利用できません。** `kimi -p` はこの
> pure-output 境界を強制できないため、spawn 前に拒否されます。Explore、
> Raw、または Quick Launcher（`/opsx:ff`）を使ってください。
...
**エンジン** — セレクターには **compatible** なプロバイダーだけが
表示され、Quick Spec では Kimi は除外されます。
