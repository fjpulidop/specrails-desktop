...
- **分析** — 日別・アクティビティ別・モデル別・チケット別の呼び出しです。Claude は請求コスト、Codex/Gemini は推定値を表示し、Kimi は取得できない token/USD 欄を空のままにします。
- **エージェント** — Claude と Kimi の provider ごとに独立した Profile と Role カタログです。Kimi Role は手動で作成・編集できますが、Generate、Test、AI Refine は利用できません。
- **コード** — 読み取り専用ファイルブラウザと、AI が触れたファイルを示すチップです。平易な AI サマリーは対応 provider でのみ表示され、Kimi では利用できません。
- **インテグレーション** — 任意のアドオンです。スペックを **Jira** ボードに接続したり、AI 向けの追加ツールを有効にしたりできます。
- **設定** — プロジェクトごとのオプション（テレメトリ、予算、プロバイダー設定など）です。

> セクションとアクションは有効な provider の capability に従います。たとえば Profile は Claude/Kimi で使えますが、Kimi では Agent Studio の AI アクションが fail closed になります。
