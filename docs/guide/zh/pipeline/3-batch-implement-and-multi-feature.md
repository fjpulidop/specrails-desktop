...
当运行结束时，这些被隔离的分支会被组装起来，并以**一个 draft pull request** 的形式，从你为项目指定的集成分支上交付出来（在 **Settings → Integration branch** 里设置它；它默认取你仓库的默认分支）。specrails **绝不合并，也绝不直接向你的集成分支提交**——你拿到的是一个供审查的 draft PR，而合并这一步由人来掌控。这是一次安全的交接：specrails 产出 pull request，你的工程师则像他们一贯做的那样，在 GitHub 里审查并合并它。

落到实处就是：

- 每个 spec 都得到一块干净的画布去实现，而不是半路接手上一个 spec 尚在进行中的编辑。
- 在运行进行期间，你的工作树绝不会被改动——在你点头之前，什么都不会落地。
- 运行完成后，你会收到一条带着 draft PR 的通知：**Open PR** 去查看它，或 **Approve** 把它提升为 ready-for-review，交给你团队日常的 GitHub 审查流程。
- 如果这些隔离的分支没法干净地合并到一起，specrails 会安全地停下来，把这些分支留给人来处理——它绝不会把一次破碎的合并硬塞到你的基线上。

> 交付 PR 需要 GitHub CLI（`gh`）已完成认证，并配置好远端。若没有这些，specrails 依然会把工作提交到一个分支上，你可以自己从它发起 pull request——不会丢失任何东西。若要回退到旧行为（在本地整合而不是发起 PR），设置 `SPECRAILS_RAIL_DELIVER_PR=0`。
