...
## Cartes de lancement et cartes d'exécution

- **L'agent propose, vous appuyez sur Play.** Demandez-lui de placer des specs sur un rail (« prépare #12 et #14 sur un rail ») et, au lieu de le décrire, il vous remet une **carte de lancement** : le rail (ou un nouveau), les specs, moteur, modèle, effort, profil, loop, PR cible et branche de base optionnelles — tout prérempli avec sa recommandation et tout modifiable. Changez ce que vous voulez et appuyez sur **Play**. Dites « lance-le maintenant » et il lance directement.
- **Une seule carte suit l'exécution.** Après Play, la proposition se fige en « Lancé → Rail N » et une **carte d'exécution** prend le relais : état, temps écoulé, ce que fait le pipeline, bouton de log et Arrêter. Quand le travail est prêt à relire, la même carte devient la carte PR habituelle (créer la PR, intégrer en local, abandonner).
- **Les échecs parlent.** Si une exécution échoue ou se bloque, la carte affiche la raison en clair et propose les vraies options — Reprendre, Récupérer, Approuver, Relancer, Abandonner — et l'agent publie un court compte rendu expliquant ce qui s'est passé et ce qu'il recommande. Il ne reprend ni ne relance jamais seul ; vous décidez depuis la carte.
