...
> **Les rails s'exécutent en parallèle.** Chaque lancement isole son travail dans un worktree git par spec, donc plusieurs rails peuvent tourner en même temps dans le même projet sans se marcher dessus — les changements de chaque exécution reviennent en merge ou en draft PR quand elle se termine.
...
Seul le chemin legacy (fonctionnalité Loops désactivée) retombe sur l'ancienne file un-job-à-la-fois par projet, où les rails supplémentaires attendent derrière celui qui tourne. Le parallélisme entre projets ne change pas : chaque projet reste totalement indépendant.
