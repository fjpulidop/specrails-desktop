...
| CRIT-4 | critical | undercount | `server/interactive-job-session.ts:210` | Interactive ultracode job finalized (or crashed) mid-turn drops the entire in-flight turn's cost — the final row can even be an authoritative $0 'success' |
...
### CRIT-4 · Interactive ultracode job finalized (or crashed) mid-turn drops the entire in-flight turn's cost — the final row can even be an authoritative $0 'success'
...
**Failure scenario.** User launches a heavy Interactive ultracode rail (the first turn IS the whole pipeline). The agent works 30-60 min; the user, seeing the work done, clicks Finalize BEFORE the turn's terminal result fires. Job Detail shows $0.00 and ai_invocations gets an authoritative-$0 success row while Claude's ledger billed $5-$15. Same loss on a mid-turn child crash.
