...
1. **Capture — wrong provider stamp on the force-fail path.** `server/modules/execution/runtime/queue-manager.ts:2083` (`_forceFailUnkillableJob`) writes the `ai_invocations` row with `provider: this._adapter.id` (the project's *primary* adapter), because the per-job provider override was already consumed-and-deleted in `_resolveJobAdapter`. On a claude-primary multi-provider project, an unkillable **codex** job is recorded as `provider='claude'` → codex run-count under-stated, claude over-stated in `byProvider`. *(BUG-ANALYTICS-01)*

2. **Capture — startup-failed jobs vanish entirely.** `server/modules/execution/runtime/queue-manager.ts:782-813` (`_failWedgedJob`) marks the job failed in `jobs.sqlite` but **never calls `recordInvocation`** — unlike its sibling `_forceFailUnkillableJob` which deliberately does. A codex/gemini project whose binary isn't on PATH (the most common new-project failure) produces a job that is *invisible* to `totalRuns`/`failureRate`. *(BUG-ANALYTICS-02)*

3. **Capture — loop steps are always `status='success'`.** `server/modules/loops/runtime/loop-run-manager.ts:341` hardcodes `status: 'success'` even though the executor hands it `r.failed`. Codex/gemini loop steps fail more often (quota/usage-limit), and every one is persisted green → `failureRate` under-counts, `avgCost`-on-success is polluted. *(BUG-ANALYTICS-03)*

4. **Capture — loop steps drop the token breakdown.** `server/modules/loops/runtime/loop-executors.ts:163,199` fold `tokens_in + tokens_out` into one scalar and discard cache tokens; `server/modules/loops/runtime/loop-run-manager.ts:345` writes it into `tokens_out` only. Total tokens under-count badly for cache-hit claude loops; for *all* providers the input/output split is corrupted. *(BUG-ANALYTICS-04)*

5. **Store — failed codex/gemini rows persist `$0 estimated` instead of `NULL`.** `server/modules/accounting/runtime/result-event.ts:64-74` stamps `cloned.model = fallbackModel` then runs the estimator over all-zero usage; `server/modules/accounting/runtime/pricing.ts:90-94` returns the number `0` (not `null`), so the row is `total_cost_usd=0, estimated=1`. The equivalent claude row is `NULL`. Asymmetry → codex/gemini failed rows leak into the scatter as `$0` dots and render `~$0.00` in the table. *(BUG-ANALYTICS-05)*
...
7. **Render — no live refresh during loop runs.** `server/modules/loops/runtime/loop-run-manager.ts:334-348` is the *only* `recordInvocation` callsite that never broadcasts `spending.invalidated`, so an open dashboard freezes during multi-hour loops (often codex/gemini). *(BUG-ANALYTICS-07)*
...
12. **Render — per-ticket spending line renders fully-estimated codex/gemini cost as authoritative.** `server/modules/accounting/runtime/ai-invocations.ts:131-184` doesn't split on the estimated flag; `TicketSpendingLine.tsx` shows a plain `$X.XX`. *(BUG-ANALYTICS-12)*
...
- **File:** `server/modules/loops/runtime/loop-run-manager.ts:341` (status hardcoded in `record()` @ 334-348); callsites @ 432 (AI Step) / 491 (Decider)
...
- **Severity:** Medium · **Layer:** capture
- **File:** `server/modules/loops/runtime/loop-executors.ts:163` (AI step) & `:199` (Decider); written in `server/modules/loops/runtime/loop-run-manager.ts:345`
...
- **File:** `server/modules/loops/runtime/loop-run-manager.ts:334-348` (`record()` helper); callers @ 432, 491
...
- **File:** `client/src/features/analytics/components/SpendingTimeline.tsx:11-19, 43-47` (verified: 5 `<Bar>` series, `isEmpty` sums 5 fields)
...
- **Severity:** Medium · **Layer:** client
- **File:** `client/src/features/analytics/components/SpendingHero.tsx:30` (`SURFACES = ['job','explore-spec','quick-spec','ai-edit','smash']`), 73-76, 138-161
...
- **File:** `server/modules/accounting/runtime/spending.ts:413-420` (topTickets build, `ticketTitle: null`, no `isDeleted`); `server/project-router-spending.ts:120` (`/spending` route returns `getSpending(...)` **with no title enrichment** — verified: unlike the `/invocations` route @ 143-153 which reads `readStore(ticketPath(req))`)
...
- **File:** `client/src/features/analytics/pages/AnalyticsPage.tsx:133` (`&limit=100`, never sends `cap`); `server/modules/accounting/runtime/spending.ts:591` (`truncated = cap !== undefined && totalRow.total > cap`); `client/.../InvocationsTable.tsx:200-204`
...
- **File:** `server/modules/execution/runtime/queue-manager.ts:2083` (`_forceFailUnkillableJob`); contrast normal path @ ~1797 (`adapter.id` from `_onJobExit`)
...
- **Severity:** Low · **Layer:** capture
- **File:** `server/modules/execution/runtime/queue-manager.ts:782-813` (`_failWedgedJob`); contrast `_forceFailUnkillableJob:2080-2092` which deliberately records
...
- **File:** `server/modules/accounting/runtime/result-event.ts:64-74` (`finaliseInvocationResult`) + `server/modules/accounting/runtime/pricing.ts:90-94`
...
- **File:** `client/src/features/analytics/components/ModelBreakdown.tsx:42` (`${m.costUsd.toFixed(2)}`, verified no `~`); server `server/modules/accounting/runtime/spending.ts:271` (`byModel` SUMs cost without carrying the estimated flag)
...
- **File:** `client/src/features/analytics/components/ProviderBreakdownCard.tsx:107-116`; root cause `server/modules/accounting/runtime/result-event.ts:75-85` (NULL cost on missing pricing entry)
...
- **File:** `client/src/features/analytics/components/ProviderBreakdownCard.tsx:88` (segment `title` uses `fmtUsd(sum)`, no `~`; verified the legend row @ ~116 prefixes `~` when `allEstimated`)
...
- **Severity:** Low · **Layer:** client
- **File:** `client/src/features/analytics/components/InvocationsTable.tsx:110-121, 178-181`
...
- **File:** `server/modules/accounting/runtime/ai-invocations.ts:131-184` (`getTicketSpendingSummary`, no estimated split); `client/src/features/analytics/components/TicketSpendingLine.tsx:64` (plain `$X.XX`)
...
- **Severity:** Low · **Layer:** client
- **File:** `client/src/features/analytics/components/CostScatter.tsx:22-28` (`COLOR` map) & `:37` (`surfaces` array) — verified both list only job/quick-spec/explore-spec/ai-edit/smash
...
- **Severity:** Low · **Layer:** client
- **File:** `client/src/features/analytics/pages/AnalyticsPage.tsx:144-161` (WS effect + `debounceRef`, cleanup @ 160)
...
- **Severity:** Low · **Layer:** aggregation + client
- **File:** `server/modules/accounting/runtime/spending.ts:294-308` (`substr(started_at,1,10)` + `eachDay`), `:150-159` (resolveRange); `client/.../SpendingTimeline.tsx:26-37`
...
- **Severity:** Low · **Layer:** client
- **File:** `client/src/features/analytics/pages/AnalyticsPage.tsx:91-97 (URL-sync), 317-322`
...
- **Severity:** Low · **Layer:** client
- **File:** `client/src/features/analytics/pages/AnalyticsPage.tsx:99-124, 306-340`
...
|-----------|---------------|---------|
| `capture-queue` | `server/modules/execution/runtime/queue-manager.ts` | BUG-ANALYTICS-01, 02 |
| `capture-loop` | `server/modules/loops/runtime/loop-run-manager.ts`, `server/modules/loops/runtime/loop-executors.ts` | BUG-ANALYTICS-03, 04, 07, 32 (started_at provenance) |
| `store-pricing` | `server/modules/accounting/runtime/result-event.ts`, `server/modules/accounting/runtime/pricing.ts` | BUG-ANALYTICS-05, 09 (server half) |
| `agg-spending` | `server/modules/accounting/runtime/spending.ts`, `server/modules/accounting/runtime/ai-invocations.ts` | BUG-ANALYTICS-08 (server), 12 (server), 18 (server half), 21 (server half), 29, 30, 31, 33, 34, 35 (server half), 36 (agg+modal) |
...
### Cluster A — `server/modules/accounting/runtime/desktop-analytics.ts` cross-project (HOME) rollup presents estimated codex/gemini cost as billed fact

> Root cause shared by 24–28: `desktop-analytics.ts` sums `COALESCE(SUM(total_cost_usd),0)` from the `jobs` table with **zero** awareness of `jobs.total_cost_usd_estimated` (db.ts migration 27; set to `1` at `db.ts:949` for non-native-cost providers). `grep total_cost_usd_estimated server/modules/accounting/runtime/desktop-analytics.ts` → 0 hits. The per-project `spending.ts` already does this split correctly (`:241`, `:464-484`) and drives an "Includes estimated costs" footnote; the cross-project rollup loses it entirely. The `jobs` table also has no provider/engine column (engine lives on `rails.ai_engine`), so this surface cannot even attribute cost to an engine. **The single shared fix** is to add a `CASE WHEN total_cost_usd_estimated=1 THEN total_cost_usd ELSE 0 END` split to `queryProjectKpi`, `queryProjectTimeline`, and `getDesktopTodayStats`, thread an `estimatedCostUsd` through, and surface a badge/footnote on `DesktopAnalyticsPage` + `StatusBar`.

#### BUG-ANALYTICS-24 ⊛ codex — HOME grand-total KPIs (`totalCostUsd` / `costToday`) blend authoritative + estimated cost as fact
- **Severity:** Medium · **Layer:** aggregation
- **File:** `server/modules/accounting/runtime/desktop-analytics.ts:93, 148, 151, 179, 183`
...
- **Trigger:** Create a project with a codex (or gemini) provider, run an implement rail (writes a `jobs` row with `total_cost_usd` from `server/modules/accounting/runtime/pricing.ts` and `total_cost_usd_estimated=1`), open the HOME / desktop analytics.
...
- **File:** `server/modules/accounting/runtime/desktop-analytics.ts:154-161, 174`
...
- **Severity:** Medium · **Layer:** aggregation
- **File:** `server/modules/accounting/runtime/desktop-analytics.ts:110, 164, 169-171`
...
- **Severity:** Medium · **Layer:** aggregation
- **File:** `server/modules/accounting/runtime/desktop-analytics.ts:250, 254`
...
- **Severity:** Low · **Layer:** aggregation
- **File:** `server/modules/accounting/runtime/desktop-analytics.ts:91-98, 34` (consumer: `DesktopAnalyticsPage.tsx:34`)
...
- **Severity:** Low · **Layer:** aggregation
- **File:** `server/modules/accounting/runtime/spending.ts:271-286`
...
- **Severity:** Low · **Layer:** aggregation
- **File:** `server/modules/accounting/runtime/spending.ts:276-282` (+ filter at `:180-184`; client click at `client/src/features/analytics/pages/AnalyticsPage.tsx:320`)
...
- **File:** `server/modules/accounting/runtime/spending.ts:180-184` (`buildWhere` model `IN`), `:272-284` (`byModel` `GROUP BY model`)
...
- **Severity:** Low · **Layer:** capture
- **File:** `server/modules/loops/runtime/loop-run-manager.ts:342` (`record()` helper, defined @ 310); called @ 432 (AI Step) / 491 (Decider) **after** the awaited step/decider resolve
...
- **Severity:** Low · **Layer:** aggregation
- **File:** `server/modules/accounting/runtime/spending.ts:339-377` (byMode query + mapping); consumed at `client/src/features/analytics/components/QuickVsExploreCard.tsx:59` (`fmtUsd(mode.avgCostPerSpec)`)
...
- **Severity:** Low · **Layer:** aggregation
- **File:** `server/modules/accounting/runtime/spending.ts:545-551` (truncation flag dead at `:591`)
...
- **Severity:** Medium · **Layer:** aggregation
- **File:** `server/modules/accounting/runtime/spending.ts:415` (card aggregation); `server/project-router-spending.ts:239-251` (summary-CSV Top-Tickets); `server/modules/accounting/runtime/ai-invocations.ts:131-166` (modal endpoint)
